...
class IStreamOfflinePluginFactoryHandler(private val originalFactory: PluginFactory): InvocationHandler {

        companion object {
            const val TAG = "IStreamOfflinePluginFactoryHandler"
    ...
    override fun invoke(proxy: Any, method: Method, args: Array<out Any>?): Any? {
        ...
        if (METHOD_NAME == method.name) {
            ...
            return createOfflinePlugin(if (args == null) ChatClient.instance().getCurrentUser() else args[0] as? User)
        }
        return method.invoke(originalFactory, args)
    }

    private fun createOfflinePlugin(user: User?): Any {
        return try {
